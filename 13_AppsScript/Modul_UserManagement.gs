...
/**
 * GET semua akun role='guru' di Kelompok akun 'admin_kelp' ini (nama, email,
 * nama Kelompok) — dipakai menu "User" Dashboard mobile Admin Kelp (BEDA dari
 * serverGetUsersList di atas yang khusus role admin_ppg/admin_desa/admin_kelompok
 * desktop, TIDAK disentuh). Akun yang sudah dihapus (status inactive) tidak
 * ikut ditampilkan. requireAdminKelpContext_ ada di Modul_InputAbsen.gs.
 */
function serverGetGuruUsersAdminKelp(token) {
  const ctx = requireAdminKelpContext_(token);
  if (!ctx.success) return ctx;

  const kelompok = readSheetAsObjects(SHEET_NAMES.KELOMPOK).find(function (k) { return k.id == ctx.kelompokId; });
  const kelompokNama = kelompok ? kelompok.nama : '';

  const list = readSheetAsObjects(SHEET_NAMES.USERS)
    .filter(function (u) {
      return u.role === 'guru' && u.scope_type === 'kelompok' && u.scope_id == ctx.kelompokId &&
        String(u.status || 'active').toLowerCase() !== 'inactive';
    })
    .map(function (u) {
      return { id: u.id, nama: u.nama || '', email: u.email || u.username || '', kelompokNama: kelompokNama };
    });
...
/**
 * DELETE (soft, sama pola dgn serverDeleteUser di atas — set status='inactive')
 * satu akun guru di Kelompok akun 'admin_kelp' ini. Login berikutnya otomatis
 * ditolak (serverLogin di Code.js sudah cek status === 'inactive').
 */
function serverDeleteGuruUserAdminKelp(token, userId) {
  const ctx = requireAdminKelpContext_(token);
  if (!ctx.success) return ctx;

  const target = readSheetAsObjects(SHEET_NAMES.USERS).find(function (u) { return u.id == userId; });
  if (!target) return { success: false, error: 'User tidak ditemukan.' };
  if (target.role !== 'guru' || target.scope_type !== 'kelompok' || target.scope_id != ctx.kelompokId) {
    return { success: false, error: 'Anda tidak memiliki akses ke akun ini.' };
  }

  withScriptLock_(function () {
    updateRowByQuery(SHEET_NAMES.USERS, { id: userId }, {
      status: 'inactive',
