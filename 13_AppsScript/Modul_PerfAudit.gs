/**
 * ⚠️⚠️⚠️ TEMPORARY — Modul_PerfAudit.gs (Tahap 6) ⚠️⚠️⚠️
 * Ditambahkan 2026-08-08 KHUSUS utk before/after measurement +
 * authorization regression test AKSES_KELAS_REQUEST_OPTIMIZATION_REPORT.md.
 * TIDAK mengubah business logic — semua fungsi di sini HANYA memanggil
 * fungsi server yang SUDAH ADA (canGuruAccessKelas_, serverAddGuru,
  ...
  if (!existing) {
    ...
    withScriptLock_(function () {
      ...
      appendRowToSheet(SHEET_NAMES.AKSES_KELAS_REQUEST, [
        ...
        'approved', '[PERFAUDIT TEMP] Tahap6 akses_kelas_request before/after — hapus setelah pengukuran selesai',
...
/** ?diag=perfaccesscheck&kelompok=&guruid=&kelas=&tanggal= — real canGuruAccessKelas_ langsung, tanpa sesi (fungsi murni). */
function diagPerfAccessCheck_(kelompokId, guruId, kelas, tanggal) {
  const t0 = Date.now();
  const allowed = canGuruAccessKelas_(kelompokId, guruId, kelas, tanggal);
  const t1 = Date.now();
...
/** ?diag=perfaksesrequestrawcount&kelompok= — hitung baris akses_kelas_request SAAT INI (read-only). */
function diagPerfAksesRequestRowCount_(kelompokId) {
  const rows = readSheetAsObjects(SHEET_NAMES.AKSES_KELAS_REQUEST).filter(function (r) { return String(r.kelompok_id) === String(kelompokId); });
