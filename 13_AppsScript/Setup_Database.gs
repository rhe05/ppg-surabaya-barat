...
function migrateSantriSchemaAddFields_(ss) {
  ...
  const newColumns = ['nama_panggilan', 'tempat_lahir', 'pendidikan', 'kelas_sekolah', 'kelas_ngaji', 'alamat', 'nama_ayah', 'nama_ibu', 'rt', 'rw', 'kelurahan', 'kode_pos', 'kabupaten_kota', 'provinsi', 'nomor_wa_ayah', 'nomor_wa_ibu', 'kecamatan'];
  const lastCol = sheet.getLastColumn();
  const existingHeaders = sheet.getRange(1, 1, 1, lastCol).getValues()[0].map(h => String(h).trim().toLowerCase());
