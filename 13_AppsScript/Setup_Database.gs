...
function setupDatabaseStructure() {
  ...
  // 3. DATA SANTRI & GURU
  createSheetIfNotExists(ss, 'santri', ['id', 'kelompok_id', 'nama', 'nis', 'gender', 'tanggal_lahir', 'jenjang_saat_ini', 'nama_panggilan', 'tempat_lahir', 'pendidikan', 'kelas_sekolah', 'kelas_ngaji', 'alamat', 'nama_ayah', 'nama_ibu', 'rt', 'rw', 'kelurahan', 'kode_pos', 'kabupaten_kota', 'provinsi']);
...
function migrateSantriSchemaAddFields_(ss) {
  ...
  const newColumns = ['nama_panggilan', 'tempat_lahir', 'pendidikan', 'kelas_sekolah', 'kelas_ngaji', 'alamat', 'nama_ayah', 'nama_ibu', 'rt', 'rw', 'kelurahan', 'kode_pos', 'kabupaten_kota', 'provinsi'];
  const lastCol = sheet.getLastColumn();
  const existingHeaders = sheet.getRange(1, 1, 1, lastCol).getValues()[0].map(h => String(h).trim().toLowerCase());
