...
function doGet(e) {
  ...
  // ⚠️⚠️ TEMPORARY (2026-08-08, Tahap 7) — dispatch diag=perf* →
  // Modul_PerfAudit.gs. HAPUS blok ini + file itu setelah before/after
  // measurement guru_izin selesai. Lihat GURU_IZIN_OPTIMIZATION_REPORT.md §Cleanup.
    ...
    try {
      ...
      switch (p.diag) {
        ...
        case 'perfsetup':
          ...
          break;
        case 'perfizincheck':
          result = diagPerfIzinCheck_(kelompokId, p.guruid, p.tanggal);
          break;
        case 'perfizinreadonly':
          result = diagPerfIzinReadOnly_(kelompokId);
          break;
        case 'perfsubmitizin':
          result = diagPerfSubmitIzin_(kelompokId, p.guruid, p.jenis, p.mulai, p.selesai, p.kategori);
          break;
        case 'perfsaveguru':
