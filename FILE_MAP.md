...
| `13_AppsScript/Modul_InputAbsen.gs` | Screen Input Absen khusus role='guru' (dikunci total, tidak lihat shell admin) — kelas guru = `jadwal_kbm.kelas` milik `guru_id` yg terhubung ke `users.guru_id`; CRUD absen per kelas (`serverGetKelasAbsenList/serverGetAbsensiKelasForm/serverSaveAbsensiKelas`) + izin akses kelas guru lain per-tanggal (sheet baru `akses_kelas_request`: `serverListKelasUntukPermintaan/serverRequestAksesKelas/serverGetIncomingAksesRequests/serverGetMyAksesRequests/serverRespondAksesRequest`) | Fitur Input Absen guru |
