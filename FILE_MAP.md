...
- Modal (semua `id="modal..."`): `modalGuruKelp` (tambah/edit guru) ·
  ...
  `modalEksporGenerus`
- `id="screenInputAbsen"` (screen Input Absen guru/admin) · `id="iaKelasRow"` (pill selector kelas, auto-select) · `id="iaIzinBtn"` (tombol "Guru Izin", disembunyikan di mode admin) · `id="modalGuruIzin"` (form Izin Harian/Cuti + alasan Sakit/Lainnya, sheet baru `guru_izin`) · `id="modalMintaAkses"`/`id="modalIncomingRequests"` (izin akses kelas guru lain)
...
- Input Absen guru (2026-07-27, redesign pill kelas + auto-select + Guru Izin): `initInputAbsen_` · `openInputAbsenAsAdmin_` · `iaOnTanggalChange_` (load kelas via spinner global, lalu AUTO-select kelas terakhir/pertama — bukan lagi tunggu klik guru) · `iaSelectKelas_` (load santri via spinner global) — chip kelas sekarang pill 1-baris (`.ia-kelas-chip-nama` + badge `.ia-kelas-chip-count`, BUKAN 2-baris nama+"N santri" lama) · Guru Izin: `iaOpenGuruIzin_`/`closeModalGuruIzin_`/`iaSetIzinJenis_` (harian/cuti)/`iaSetIzinAlasan_` (sakit/lainnya, toggle field detail)/`iaSubmitGuruIzin_` → server `serverSubmitGuruIzin`/`serverGetGuruIzinAlasanSuggestions`
