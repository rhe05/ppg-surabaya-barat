...
| `13_AppsScript/Modul_MaintainPengurus.gs` | UPSERT Data Pengurus (Data Master, sheet baru `pengurus_kelp`, DI ATAS Data Guru) — 7 jabatan tetap (`JABATAN_PENGURUS_`: Pembina/Wk Pembina Generus Kelp, PJP Kelp, Kepsek, Pembina Pra Remaja/Remaja, Ketua Muda-Mudi), 1 orang per jabatan per Kelompok (bukan riwayat), nama BEBAS diketik (bukan pilih dari Guru/Generus existing) | Fitur Data Pengurus |
...
- Data Pengurus (Data Master, sheet baru `pengurus_kelp`, DI ATAS Data Guru — bukan pilih generus existing, nama bebas diketik): `loadPengurusList_` · `togglePengurusDashTable_` · `renderPengurusDashTable_` · `renderPengurusKpis_` (isi tiap kartu jabatan dari `PENGURUS_JABATAN_MAP_`) · `openModalPengurus`/`editPengurus`/`savePengurus`/`deletePengurus` · `confirmEksporPengurus`.
