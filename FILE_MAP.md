...
- ⚠️ Perf "Pilih Kelas" (2026-07-28, ERROR_LOG #18+#19): `santri`/`guru`/`jadwal_kbm` Kelp Petemon (`kelompok_id: '1'`) sudah pindah Firestore (`FIRESTORE_KELOMPOK_TABLES_`, Modul_Utilities.gs) — tiap `readSheetAsObjects()` di tabel itu = request Firestore, JANGAN dipanggil di dalam loop ATAU berulang dalam 1 fungsi. `getKelasOwnedByGuru_`/`getKelasSessionInfo_` terima param opsional array-yang-sudah-dibaca (`jadwalRowsAll`/`guruRowsAll`) — SELALU baca sheet dulu baru panggil fungsi ini, bukan sebaliknya. `serverGetKelasAbsenList`/`serverGetKelasAbsenListAdmin` terima `preferKelas`, return `formKelas`+`formData` sekalian (gabung 2 round-trip client↔server jadi 1) — `iaLoadKelasList_` (Script_Main.html) pakai `result.formData` langsung kalau cocok, skip panggil `serverGetAbsensiKelasForm` terpisah.
