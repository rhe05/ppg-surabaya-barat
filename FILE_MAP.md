...
- **Struktur Pengurus** (bagan CSS-only, tombol di toolbar Daftar Pengurus): `openModalStrukturPengurus`/`closeModalStrukturPengurus`/`renderStrukturPengurusChart_`/`strukturNodeHtml_` — Pembina Generus → Wk Pembina Generus (1-4 kotak sejajar, dapukan multi-holder) → PJP Kelp → 5 kotak (Kepsek/Pembina Pra Remaja/Pembina Remaja/Koord Tahfidz/Sekr Generus). Garis bagan murni CSS border (`.struktur-branch-row` border-top + `.struktur-node-wrap::before` kaki vertikal), bukan SVG/gambar.
