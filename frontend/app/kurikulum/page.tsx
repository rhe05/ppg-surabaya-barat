...
/* Ganti 2-4 tombol teks yang dulu selalu terlihat (Ubah/Hapus) jadi satu
   ...
   (dihitung dari getBoundingClientRect() tombol), BUKAN absolute relatif
   ke tombolnya spt draf pertama -- draf pertama ke-clip tak kelihatan di
   baris Probul TERAKHIR tiap semester (Des utk semester akademik Jul-Des,
   Jun utk Jan-Jun), krn kartu Prota & wrapper animasi buka/tutup Promes
   sama2 overflow-hidden (perlu utk clip sudut rounded & trik animasi
   grid-rows 0fr<->1fr) dan box-nya berhenti PERSIS di baris terakhir --
   tidak ada slack vertikal utk dropdown yg terbuka ke bawah. position:fixed
   lolos dari overflow-hidden leluhur manapun (containing block-nya viewport,
   bukan box tsb), jadi otomatis benar di semua baris termasuk yg terakhir. */
                    ...
                    {namaAsli === KATEGORI_BACAAN_ALQURAN ? (
                      ...
                      <>
                        {p.target && (
                          <div className="mt-1">
                            <div className="text-[10.5px] font-bold text-sage">Semester 1</div>
                            <div className="mt-0.5 whitespace-pre-line text-[12px] text-text">{p.target}</div>
                          </div>
                        )}
                        {p.target2 && (
                          <div className="mt-2 border-t border-border pt-2">
                            <div className="text-[10.5px] font-bold text-sage">Semester 2</div>
                            <div className="mt-0.5 whitespace-pre-line text-[12px] text-text">{p.target2}</div>
                          </div>
                        )}
                      </>
                    ) : (
