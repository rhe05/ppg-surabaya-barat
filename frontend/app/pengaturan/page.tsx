...
function PengaturanContent() {
  ...
  /* ── Registrasi Guru (2026-08-24) -- konsep SAMA PERSIS "Registrasi"
     admin kelp di atas, tapi utk admin_kelompok mendaftarkan guru
     kelompoknya sendiri. TIDAK ada tabel/RPC klaim baru -- ini cuma cara
     ringkas mengisi tabel `guru` yang sudah ada (guru_insert_admin RLS
     sudah mengizinkan tiga peran ini), guru-nya klaim akun sendiri lewat
     mekanisme yang SUDAH ADA (cari_guru_untuk_klaim/klaim_akun_guru,
     20260820110000). Berbagi `kelompokId` yang sama dgn Kop Surat/
     Kalender di atas. */
  ...
  const muatGuruRegistrasi = useCallback(async () => {
    ...
    /* Status klaim (siapa yang SUDAH menghubungkan akun) tidak bisa
       dibaca langsung dari tabel `profiles` (RLS-nya "id = auth.uid()"
       saja, admin pun tidak boleh baca profil orang lain) -- lewat RPC
       SECURITY DEFINER yang memeriksa wewenang scope pemanggil sendiri
       (status_klaim_guru, migrasi 20260824120000). */
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      ...
      <p className="mb-6 text-[13px] text-text-dim">
        Kutipan harian, kop surat untuk laporan PDF, kalender kelompok, registrasi admin kelp, dan
        registrasi guru.
      </p>
      ...
      {bolehDaftarGuru && (
        <div className="mb-8 rounded-card border border-border bg-panel p-5 shadow-[var(--shadow-card)]">
          <div className="mb-4 text-[15px] font-bold text-text">Registrasi Guru</div>
          ...
          {kelompokId && (
            <>
              <div className="mb-4 flex flex-wrap items-end gap-3 rounded-[var(--radius)] border border-border bg-panel-2 p-3.5">
                ...
                <button
                  onClick={daftarkanGuru}
                  disabled={sibuk || !namaGuruBaru.trim()}
                  className={KELAS_TOMBOL_UTAMA}
                >
                  Daftar
                </button>
                          ...
                          <div className="mt-1.5">
                            {sudahBergabung.map((g) => (
                              ...
                              >
                                ...
                                {g.klaimPada && (
                                  <div className="mt-1 text-[11px] text-text-faint">
                                    Bergabung {formatWaktuBergabung(g.klaimPada)}
                                  </div>
                                )}
                              </div>
                            ))}
